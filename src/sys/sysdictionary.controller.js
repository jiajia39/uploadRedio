...
const controller = (() => {
   ...
   *     security:
   *       - Authorization: []
   *     description: sysdictionary get all
   *     tags: [sysdictionary]
   *     produces:
   *       - application/json
   *     parameters:
   *       - name: cGuid
   *         description: sysdictionary's id.
   *         in: query
   *         type: string
   *       - name: cCategory
   *         description: sysdictionary's cCategory.
   *         in: query
   *         type: string
   *     responses:
   *       200:
  ...
  router.get('/getall', async (req, res) => {
    const { cGuid, cCategory } = req.query;

    const filter = { OR: [] };

    if (cGuid) filter.OR.push({ cGuid });
    if (cCategory) filter.OR.push({ cCategory });

    if (filter.OR.length < 1) {
      const data = await prisma.Sys_Dictionary.findMany();

      res.json({ data, message: 'Data obtained.' });
    } else {
      const data = await prisma.Sys_Dictionary.findMany({ where: filter });
  ...
  router.get('/item/:cGuid', async (req, res) => {
    const data = await prisma.Sys_Dictionary.findUnique({
  ...
  router.get('/count', async (req, res) => {
    const data = await prisma.Sys_Dictionary.count();
    res.json({ data, message: 'Data obtained.' });
  });
   ...
   *     responses:
   *       200:
   *         description: sysdictionary/pagination
   *         schema:
   *           type: object
   */
  router.get('/pagination', async (req, res) => {
    ...
    const count = await prisma.Sys_Dictionary.count();
    if (count != null && count > 0) {
      const rstdata = await prisma.Sys_Dictionary.findMany({
        skip: (page - 1) * row,
        take: row,
        orderBy: [
          {
  ...
  router.post('/add', async (req, res) => {
    ...
    await prisma.Sys_Dictionary({ data: req.body });

    res.json({ isok: true, message: 'SysDictionary saved' });
  });
  ...
  router.put('/item/:cGuid', async (req, res) => {
    const message = await prisma.Sys_Dictionary.update({
      where: { cGuid: req.params.cGuid },
      data: req.body,
    }).then(() => 'List updated');
  ...
  /**
   * @swagger
   * /api/sys/sysdictionary/delete/{cGuid}:
   *   delete:
   *     security:
   *       - Authorization: []
   *     description: Login to the application
  ...
  router.delete('/delete/:cGuid', async (req, res) => {
    const message = await prisma.Sys_Dictionary.delete({
      where: { cGuid: req.params.cGuid },
    }).then(() => 'SysDictionary deleted');
  ...
  /**
   * @swagger
   * /api/sys/sysdictionary/deleteids:
   *   delete:
   *     security:
   *       - Authorization: []
   *     description: Login to the application
...
controller.prefix = '/sys/sysdictionary';
